...
const ProductForm = ({ onSubmit, isSubmitting }) => {
  ...
  // Sync the default category once categories load
  useEffect(() => {
  ...
  const handleSubmit = async (data) => {
    try {
      console.log("Submitting product data:", data); // Debug: Check form data
      await onSubmit(data);
      form.reset({
        category: "",
        image: "",
      ...
      <div className="border p-6 rounded-lg shadow-sm bg-white w-full max-w-2xl">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="grid grid-cols-2 gap-4">
            ...
            {/* Stock Field (renamed from Inventory) */}
            <FormField
              control={form.control}
              name="stock" // Changed to "stock"
              render={({ field }) => (
                <FormItem>
...
//                             field.onChange("");
//                           } else {
//                             const numberValue = Math.max(0, parseFloat(value));
...
//                           field.onChange("");
//                         } else {
//                           const numberValue = Math.max(0, parseInt(value));
//                           field.onChange(numberValue);
//                         }
//                       }}
//                       value={field.value === 0 ? "" : field.value} // Hide zero
//                     />
//                   </FormControl>
//                   <FormMessage />
//                 </FormItem>
