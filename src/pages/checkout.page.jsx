...
import { useCreateOrderMutation } from "@/lib/api"; // Import RTK Query mutation hook

function CheckoutPage() {
  const cart = useSelector((state) => state.cart.value);
  const [createOrder, { isLoading: loadingOrder, error: orderError }] = useCreateOrderMutation(); // Use mutation hook
  const [loading, setLoading] = useState(false); // Additional loading state for UI
  const navigate = useNavigate();

  // Calculate subtotal and total
  const subtotal = cart.reduce((acc, item) => acc + item.product.price * item.quantity, 0);
  const shippingCost = 100.0; // Fixed shipping fee
  ...
  const handleCheckout = async (shippingAddress) => {
    ...
    try {
      const orderData = {
        items: cart.map((item) => ({
          product: item.product._id, // Match backend expectation
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="border p-6 rounded-lg shadow-sm bg-white">
          ...
          <ShippingAddressForm
            ...
            loading={loading || loadingOrder} // Combine loading states
          />
          {orderError && (
...
//   // Calculate subtotal and total
//   const subtotal = cart.reduce((acc, item) => acc + item.product.price * item.quantity, 0);
//   const shippingCost = 100.00; // Fixed shipping fee
//   const total = subtotal + shippingCost;
